import React from "react";
import AddContent from "./AddContent";
import AddDisplay from "./AddDisplay";

const Dashboard = () => {
  const a = [1, 2, 3, 4, 5, 6, 55];
  return (
    <div>
      {/* <AddDisplay /> */}
      <AddContent />
      <header class="text-gray-600 body-font">
        <div class="container mx-auto flex flex-wrap p-5 flex-col md:flex-row items-center">
          <a class="flex title-font font-medium items-center text-gray-900 mb-4 md:mb-0">
            <span class="ml-3 text-xl dark:text-white">Everidoor</span>
          </a>
          <nav class="md:ml-auto flex flex-wrap items-center text-base justify-center">
            <a class="mr-5 hover:text-gray-900">Advertiser</a>
            <a class="mr-5 hover:text-gray-900">Display Provider</a>
            <a class="mr-5 hover:text-gray-900">INR 17000</a>
            <a class="mr-5 hover:text-gray-900">Welcome</a>
          </nav>
          <div
            class=" inline-flex dark:bg-white border-black text-black dark:text-gray-800 mx-2 items-center justify-center px-4 py-2 mt-4 text-lg font-bold transition-all duration-200 bg-transparent border border-transparent sm:mt-0 font-pj rounded-lg focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-300 hover:bg-black hover:text-white focus:bg-gray-200"
            role="button"
          >
            Jane
            <svg
              xmlns="http://www.w3.org/2000/svg"
              class="h-3 w-3 ml-3"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              stroke-width="2"
            >
              <path
                stroke-linecap="round"
                stroke-linejoin="round"
                d="M19 9l-7 7-7-7"
              />
            </svg>
          </div>
        </div>
        <hr />
      </header>
      <div class="h-screen flex">
        <main
          class="flex-1 bg-gray-200 dark:bg-gray-900 overflow-y-auto transition
		duration-500 ease-in-out"
        >
          <div
            class="px-24 py-12 text-gray-700 dark:text-gray-500 transition
			duration-500 ease-in-out"
          >
            <div class="flex items-center justify-between my-4 select-none">
              <h2 class="text-4xl font-medium capitalize">Display Dashboard</h2>

              <button
                type="button"
                className="inline-flex items-center justify-center px-6 py-3 text-sm font-semibold leading-5 text-white transition-all duration-200 bg-blue-600 border border-transparent rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-600 hover:bg-blue-500"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="h-6 w-6"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                  stroke-width="2"
                >
                  <path
                    stroke-linecap="round"
                    stroke-linejoin="round"
                    d="M12 6v6m0 0v6m0-6h6m-6 0H6"
                  />
                </svg>
                Add a display
              </button>
            </div>
            <div class="mt-1 mb-4 flex items-center justify-start">
              <span class="text-sm">Show Displays</span>
              <button
                type="button"
                className="inline-flex items-center justify-center px-4 py-1 mx-2 text-sm font-semibold leading-5 text-white transition-all duration-200 bg-blue-600 border border-transparent rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-600 hover:bg-blue-500"
              >
                Campaign Mode
              </button>
              <button
                type="button"
                className="inline-flex items-center justify-center px-4 py-1 mx-2 text-sm font-semibold leading-5 text-white transition-all duration-200 bg-blue-600 border border-transparent rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-600 hover:bg-blue-500"
              >
                My content Mode
              </button>
              <button
                type="button"
                className="inline-flex items-center justify-center px-4 py-1 mx-2 text-sm font-semibold leading-5 text-gray-600 transition-all duration-200 bg-white border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 hover:bg-gray-50 hover:text-gray-900"
              >
                Archived
              </button>
            </div>
            <div
              class="border dark:border-gray-700 transition duration-500
				ease-in-out"
            ></div>
            <div class="flex flex-col mt-2">
              {a.map((item) => (
                <div class="flex flex-row mt-2 relative">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    class="h-6 w-6 absolute right-4 text-gray-500 top-3"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    stroke-width="2"
                  >
                    <path
                      stroke-linecap="round"
                      stroke-linejoin="round"
                      d="M5 12h.01M12 12h.01M19 12h.01M6 12a1 1 0 11-2 0 1 1 0 012 0zm7 0a1 1 0 11-2 0 1 1 0 012 0zm7 0a1 1 0 11-2 0 1 1 0 012 0z"
                    />
                  </svg>
                  <div
                    class="flex w-full items-center justify-between bg-white
         dark:bg-gray-800 px-8 py-6 border-l-4 border-green-500
         dark:border-green-300"
                  >
                    <div class="flex">
                      <img
                        class="h-full w-40 object-cover"
                        src="data:image/jpeg;base64,"
                        alt="infamous"
                      />

                      <div class="flex flex-col ml-6">
                        <span class="text-lg font-bold">
                          Digital display, veags mall , dwarka
                        </span>
                        <div class="mt-4 flex">
                          <div class="flex">
                            <svg
                              class="h-5 w-5 fill-current
                             dark:text-gray-300"
                              viewBox="0 0 24 24"
                            >
                              <path
                                d="M12 4a4 4 0 014 4 4 4 0 01-4 4
                                 4 4 0 01-4-4 4 4 0 014-4m0
                                 10c4.42 0 8 1.79 8
                                 4v2H4v-2c0-2.21 3.58-4 8-4z"
                              ></path>
                            </svg>
                            <span
                              class="ml-2 text-sm text-gray-600
                             dark:text-gray-300 capitalize"
                            >
                              101
                            </span>
                          </div>

                          <div class="flex ml-6">
                            <svg
                              class="h-5 w-5 fill-current
                             dark:text-gray-300"
                              viewBox="0 0 24 24"
                            >
                              <path
                                d="M19
                                 19H5V8h14m-3-7v2H8V1H6v2H5c-1.11
                                 0-2 .89-2 2v14a2 2 0 002 2h14a2 2
                                 0 002-2V5a2 2 0 00-2-2h-1V1m-1
                                 11h-5v5h5v-5z"
                              ></path>
                            </svg>
                            <span
                              class="ml-2 text-sm text-gray-600
                             dark:text-gray-300 capitalize"
                            >
                              Qweens airlines
                            </span>
                          </div>
                        </div>
                      </div>
                    </div>

                    <div class="flex flex-col -mt-10 mr-20">
                      <span
                        class="font-semibold text-green-500
                 dark:text-green-300"
                      >
                        Todays Earnings
                      </span>
                      <span
                        class="font-semibold text-green-500
                 dark:text-green-300"
                      >
                        200 $
                      </span>
                    </div>
                  </div>
                </div>
              ))}
            </div>
          </div>
        </main>
      </div>
    </div>
  );
};

export default Dashboard;
